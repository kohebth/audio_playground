...
double fast_exp(double x) {
    ...
    const double m_2pw_f = ((((((c6 * f + c5) * f + c4) * f + c3) * f + c2) * f + c1) * f + c0);
    ...
    if (i < -1022)
        ...
        memcpy(&m_2pw_i, &bits, sizeof(double));
    }
    return m_2pw_f * m_2pw_i;
}
...
double fast_atan(const double x) {
    double ax = fabs(x);
    double result;
    if (ax > 1.0) {
        ax = fast_recip(ax);
...
double fast_sigmoid(const double x) {
    return fast_recip(1.0 + fast_exp(-x));
}

double fast_db2ratio(const double val_dB) {
    return fast_exp(M_LOG10E * 0.05 * val_dB);
}

double fast_f2w(const double f) {
    return 2.0 * M_PI * f;
}

double fast_t2w(const double t) {
